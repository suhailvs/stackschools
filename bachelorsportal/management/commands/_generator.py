...
SITEMAP_HOST = 'stackschools.com'
# Port used in sitemap links
SITEMAP_PORT = '443' #'443'
# Protocol used in sitemap links
SITEMAP_PROTO = 'https' # https

# Default directory in media storage where sitemaps are stored
SITEMAP_MEDIA_PATH = 'sitemaps'
SITEMAP_MAPPING = 'mysite.urls.my_sitemaps'

# Default media storage for sitemaps
SITEMAP_STORAGE = 'django.core.files.storage.default_storage'

# Default name of sitemap index view
SITEMAP_INDEX_NAME = 'django.contrib.sitemaps.views.index'
...
class ResponseRecorder:
    ...
    def record(self, url: str) -> bytes:
        ...
        environ = {
            ...
            'wsgi.run_once': False,
            'wsgi.url_scheme': SITEMAP_PROTO,
            'SCRIPT_NAME': '',
            'SERVER_PROTOCOL': 'HTTP/1.1',
            'SERVER_NAME': SITEMAP_HOST,
            'SERVER_PORT': SITEMAP_PORT,
            'PATH_INFO': url.path,
            'QUERY_STRING': url.query,
            'HTTP_X_FORWARDED_PROTO': SITEMAP_PROTO
        }
...
class SitemapGenerator:
    ...
    def __init__(self,
        ...
        self.sitemap_root = media_path or SITEMAP_MEDIA_PATH

        if storage is None:
            storage = import_string(SITEMAP_STORAGE)

        self.storage = storage
        self.index_url_name = index_url_name or SITEMAP_INDEX_NAME
        sitemaps_view_name = sitemaps_view_name or SITEMAPS_VIEW_NAME
        self.sitemaps_view_name = sitemaps_view_name

        if sitemaps is None:
            sitemaps = import_string(SITEMAP_MAPPING)
